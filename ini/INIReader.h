...
class INIReader {
  ...
  protected:
    ...
    template <typename T> T Converter(std::string s) const;
...
INIReader::Get(std::string section) const {
...
template <typename T> inline T INIReader::Converter(std::string s) const {
    try {
        T v{};
